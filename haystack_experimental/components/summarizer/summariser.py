...
class Summarizer:
    ...
    def combine_chunks_with_no_minimum(
        ...
        :param chunks:
        :param max_tokens:
        :type chunk_delimiter:
        :type header:
        :type add_ellipsis_for_overflow:
        """

        dropped_chunk_count = 0
        output = []  # list to hold the final combined chunks
        output_indices = []  # list to hold the indices of the final combined chunks
        candidate = ([] if header is None else [header])  # list to hold the current combined chunk candidate
        candidate_indices = []
    ...
    def summarize(
        ...
        if verbose:
            ...
            print(f"Chunk lengths are {[len(self.tokenize(x)) for x in text_chunks]}")
    ...
    def run(self, documents: List[Document], detail: float = 0.05):
        ...
        for doc in documents:
            summary = self.summarize(doc.content, detail=detail, model=self.model)
