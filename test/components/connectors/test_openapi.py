...
class TestOpenAPIServiceConnector:
    @pytest.fixture
    def setup_mock(self):
        with patch("haystack_experimental.components.connectors.openapi.OpenAPIServiceClient") as mock_client:
    ...
    def test_init(self):
        ...
        assert service_connector.llm_provider == "openai"

    def test_init_with_anthropic_provider(self):
        service_connector = OpenAPIServiceConnector(provider="anthropic")
        assert service_connector is not None
        assert service_connector.llm_provider == "anthropic"
