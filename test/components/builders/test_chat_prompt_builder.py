...
from haystack_experimental.components.builders.chat_prompt_builder import (
    ChatPromptBuilder,
)
...
class TestChatPromptBuilder:
    def test_init(self):
        builder = ChatPromptBuilder(
            template=[
                ChatMessage.from_user(text="This is a {{ variable }}"),
                ChatMessage.from_system(text="This is a {{ variable2 }}"),
            ]
        )
    ...
    def test_init_with_required_variables(self):
        builder = ChatPromptBuilder(
            template=[ChatMessage.from_user("This is a {{ variable }}")],
    ...
    def test_run_template_variable_overrides_variable(self):
        ...
        res = builder.run(
            template_variables={"variable": "test_from_template_var"}, variable="test"
        )
        assert res == {
    ...
    def test_run_without_input(self):
        ...
        res = builder.run()
        assert res == {
            "prompt": [ChatMessage.from_user("This is a template without input")]
        }
    ...
    def test_run_with_variables(self):
        ...
        expected_result = {
            "prompt": [ChatMessage.from_user("Hello, John! How are you?")]
        }

        assert (
            builder.run(template_variables=template_variables, var1="How are you?")
            == expected_result
        )
    ...
    def test_run_without_template(self):
        ...
        with pytest.raises(
            ValueError,
            match="The ChatPromptBuilder requires a non-empty list of ChatMessage instances",
        ):
            prompt_builder.run()
    ...
    def test_chat_message_list_with_mixed_object_list(self):
        prompt_builder = ChatPromptBuilder(
            template=[ChatMessage.from_user("Hello"), "there world"],
            variables=["documents"],
        )
        with pytest.raises(
            ValueError,
            match="The ChatPromptBuilder expects a list containing only ChatMessage instances",
        ):
            prompt_builder.run()

    def test_provided_template_variables(self):
        prompt_builder = ChatPromptBuilder(
            variables=["documents"], required_variables=["city"]
        )
    ...
    def test_example_in_pipeline(self):
        default_template = [
            ChatMessage.from_user(
                "Here is the document: {{documents[0].text}} \\n Answer: {{query}}"
            )
        ]
        prompt_builder = ChatPromptBuilder(
            template=default_template, variables=["documents"]
        )
        ...
        template = [
            ChatMessage.from_user(
                "Here is the document: {{documents[0].content}} \n Query: {{query}}"
            )
        ]
        result = pipe.run(
            data={
    ...
    def test_example_in_pipeline_simple(self):
        default_template = [
            ChatMessage.from_user("This is the default prompt:\n Query: {{query}}")
        ]
...
class TestChatPromptBuilderDynamic:
    def test_multiple_templated_chat_messages(self):
        ...
        messages = [
            ChatMessage.from_system(
                "Write your response in this language:{{language}}"
            ),
    ...
    def test_multiple_templated_chat_messages_in_place(self):
        ...
        messages = [
            ChatMessage.from_system(
                "Write your response ins this language:{{language}}"
            ),
            ChatMessage.from_user("Tell me about {{location}}"),
        ]
    ...
    def test_some_templated_chat_messages(self):
        ...
        expected_messages = [
            ChatMessage.from_system(
                "Please, respond in the following language: English."
            ),
        ...
        assert (
            result["prompt"] == expected_messages
        ), "The templated messages should match the expected output."
        ...
        assert res == {
            "prompt_builder": {
                "prompt": [
                    ...
                    ChatMessage.from_user(
                        "What's the weather forecast for Berlin in the next 5 days?"
                    ),
                ]
            }
        }
    ...
    def test_pipeline_complex(self):
        ...
        pipe = Pipeline()
        pipe.add_component(
            "prompt_builder", ChatPromptBuilder(variables=["value_output"])
        )
        ...
        assert res == {
            "prompt_builder": {
                "prompt": [
                    ChatMessage.from_system(
                        "You give valuable information to tourists."
                    ),
                    ChatMessage.from_user("Tell me about Berlin"),
                ]
            }
